...
/datum/round_event/stray_meteor/announce(fake)
	if(length(GLOB.meteor_list))
		...
		priority_announce("Наши сенсоры обнаружили [sensor_name] тип метеорита, приближающийся к [GLOB.station_name]. Пожалуйста, приготовьтесь к столкновению.", "ВНИМАНИЕ: МЕТЕОРЫ")
