...
/datum/sm_delam/cascade/delaminate(obj/machinery/power/supermatter_crystal/sm)
	...
	priority_announce("В вашем секторе произошло событие резонансного сдвига типа C. Сканирование показывает локальный колебательный поток, влияющий на пространственную и гравитационную структуру. \
		Образовалось несколько резонансных точек. Пожалуйста, будьте бдительны.", "ВНИМАНИЕ: АНОМАЛИЯ", ANNOUNCER_SPANOMALIES)
	sleep(2 SECONDS)
...
/datum/sm_delam/cascade/proc/announce_cascade(obj/machinery/power/supermatter_crystal/sm)
	...
	if(!can_select(sm))
		return FALSE
	priority_announce("Внимание: сканирование аномалий на больших расстояниях показывает аномальные количества гармонического потока, исходящего от \
	субъекта в пределах [station_name()]. Может произойти резонансный коллапс.",
	"ВНИМАНИЕ: АНОМАЛИЯ", 'sound/announcer/alarm/airraid.ogg')
	return TRUE
